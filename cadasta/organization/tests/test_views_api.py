...
class OrganizationProjectListAPITest(TestCase):
    ...
    def test_project_private_visibility(self):
        organization = OrganizationFactory.create(**{'slug': 'namati'})
        ProjectFactory.create(**{'name': 'opdp', 'access': "private",
                                 'organization': organization})

        request = APIRequestFactory().get(
            '/v1/organizations/namati/projects/?access=private'
        )
...
class ProjectDetailTest(TestCase):
    ...
    def test_get_project(self):
        ...
        project = ProjectFactory.create(**{'project_slug': 'project',
                                           'organization': organization})
        request = APIRequestFactory().get(
            '/v1/organizations/namati/projects/{slug}'.format(
    ...
    def test_valid_update(self):
        ...
        request = APIRequestFactory().patch(
            '/v1/organizations/namati/projects/{slug}/'.format(
                slug=project.project_slug), data)
        force_authenticate(request, user=self.user)
