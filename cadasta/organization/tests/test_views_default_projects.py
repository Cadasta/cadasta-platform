...
class ProjectMapTest(FileStorageTestCase, ViewTestCase, UserTestCase,
    ...
    def test_get_with_unauthorized_user_with_redirect(self):
        user = UserFactory.create()
        spatial_unit = SpatialUnitFactory.create(project=self.project)
        project_dashboard = reverse('organization:project-dashboard', kwargs={
            'organization': self.project.organization.slug,
            'project': self.project.slug,
        })
        location_detail = reverse('async:spatial:detail', kwargs={
            'organization': self.project.organization.slug,
            'project': self.project.slug,
            'location': spatial_unit.id,
        })
        self.client.force_login(user)
        response = self.client.get(
            location_detail, follow=True, HTTP_REFERER=project_dashboard)
        assert response.status_code == 200
        assert 'permission-error' in response._headers

    def test_get_with_anonymous_user_with_redirect(self):
        spatial_unit = SpatialUnitFactory.create(project=self.project)
        project_dashboard = reverse('organization:project-dashboard', kwargs={
            'organization': self.project.organization.slug,
            'project': self.project.slug,
        ...
        response = self.client.get(
            location_detail, follow=True, HTTP_REFERER=project_dashboard)
        assert response.status_code == 200
        assert 'anonymous-user' in response._headers
    ...
    def test_get_with_locations_extent(self):
        ...
        response = self.request(user=self.user)
        assert response.status_code == 200
        expected = self.render_content(locations_count=2)
        assert response.content == expected
